...
class UniformPairWithoutReplacement(Sampler):
    ...
    def generate_samples(self,data,max_samples=None):
        self.init(data,max_samples)
        idxs = list(range(self.data.nnz))  # debug
        random.shuffle(idxs)
        ...
        for _ in range(self.num_samples(self.data.nnz)):
            ...
            # print("u: ", u)
            # print("self.data[u]", self.data[u].shape)
            i = self.items[self.idx]

            user_items = self.data[u]
            if len(user_items.shape) > 1:
...
if __name__ == '__main__':
    ...
    mtx_file_path = '../preprocess/sparse_matrix.mtx'
    data = mmread(mtx_file_path).tocsr()
