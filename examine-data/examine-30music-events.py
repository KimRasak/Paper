...
import numpy as np
# 检查数据的格式:
# 1. events中每一行都能读到user id和track id
    ...
    with open(file_path_persons) as f:
        for line in f:
            ...
            if not str.isnumeric(pid):
                raise Exception("Error! extracted string isn't number")
            pids.add(int(pid))
    return pids
    ...
    with open(file_path_events) as f:
        ...
        for line in f:
            ...
            if (uid not in uids) and (uid not in unexpected_uids):
                print("Error! current extracted 【user id】 %d isn't in stored 【user ids】.\n"
                                "line: %s" % (uid, line))
                unexpected_uids.append(uid)

            if (tid not in tids) and (tid not in unexpected_tids):
                print("Error! current extracted 【track id】 %d isn't in stored 【track ids】.\n"
                                "line: %s" % (tid, line))
